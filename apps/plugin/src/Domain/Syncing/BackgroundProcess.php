...
abstract class BackgroundProcess extends AsyncRequest
{
	...
	/**
	 * Stats for the background process
	 *
	 * @var array
	 */
	protected $stats = [
	...
	public function __construct($allowed_batch_data_classes = true)
	{
		...
		$this->stats_key = $this->identifier . '_stats';
		$this->load_stats();
	}
	...
	{
		$saved_stats = get_site_option($this->stats_key, []);
	...
	{
		update_site_option($this->stats_key, $this->stats);
	}
	...
	{
		$this->stats = wp_parse_args($stats, $this->stats);
		$this->save_stats();
	}
	 ...
	 *
	 * @param array $process_info Information about the completed process
	 */
	protected function record_process($process_info)
	{
		$this->stats['last_process'] = wp_parse_args($process_info, [
	...
	{
		// Record final stats before resetting
		$this->record_process([
		...
		$this->stats['items_processed'] = 0;
		$this->stats['remaining_items'] = 0;
		$this->save_stats();
	 ...
	 * }
	 */
	public function get_process_status(): array
	{
		$status = 'idle';
		if ($this->is_cancelled()) {
			$status = 'cancelled';
			...
			$status = 'paused';
		} elseif ($this->is_processing()) {
			$status = 'processing';
		} elseif ($this->is_queued()) {
			$status = 'queued';
		}
		...
		return [
			...
			'total_items'     => $this->stats['total_items'],
			'items_processed' => $this->stats['items_processed'],
			'remaining_items' => $this->stats['remaining_items'],
			'status'         => $status,
			'last_process'   => $this->stats['last_process']
		];
	}
}
